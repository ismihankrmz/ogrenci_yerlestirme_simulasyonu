...
def memnuniyet_puani(ogrenci):
    ...
    try:
        ...
        return 0  # Tercih dışı (Ek Kontenjan) atama
...
def veri_uret(firma_sayisi, ogrenci_sayisi, tohum, csv_yolu, json_yolu):
    ...
    for i, (idx, _) in enumerate(ogrenciler.iterrows()):
        ...
        uygun_sirali = uygun_f_df.sort_values(
            by="gno_sarti", ascending=False
        ).index.tolist()
        ...
        if len(uygun_firmalar) >= 15:
            ...
            digerleri = (
                random.sample(uygun_sirali[:-3], 2)
                if len(uygun_sirali) > 3
                else random.sample(uygun_sirali, 2)
            )
            ...
            en_dusukler = firmalar.sort_values(
                by="gno_sarti", ascending=True
            ).index.tolist()
    ...
    for idx in bos_idx:
        ...
        for sira, fid in enumerate(ogr["tercihler"], 1):
            ...
            if float(ogr["gno"]) < f_sart:
                ...
                puan = 5 - (sira - 1)
                log_func(
                    f"    {f_ad:<20} -> KABUL \u2705 ({sira}. Tercih) | Puan: {puan}\n",
                    "kabul",
                )
                break
...
def heuristik_turu(ogrenciler, firmalar, tur_no, log_func):
    ...
    for idx in bos_idx:
        ...
        for sira, fid in enumerate(ogr["tercihler"], 1):
            ...
            skor = (5 - (sira - 1)) * 10 + (float(ogr["gno"]) * 2)
            log_func(
                f"    {f_ad:<20} -> ADAY   " f"({sira}. Tercih | Skor: {skor:.1f})\n",
                "info",
            )
...
def simulasyon_motoru(yontem, ogrenciler, firmalar, red_orani, log_func, swap_log=None):
    ...
    # 1. AŞAM: Tercihlere Göre Yerleştirme
    ...
    if acikta:
        ...
        for idx in acikta:
            ...
            if uygun_f:
                ...
                log_func(
                    f"  {ogrenciler.at[idx,'ad']} -> {firmalar.at[fid,'ad']} (Ek Kontenjan Ataması)\n",
                    "info",
                )
...
class BTU_App(tk.Tk):
        ...
        if dosya_yolu:
            try:
                ...
                rapor_df["Atanan_Firma"] = rapor_df["atanan_firma"].apply(
                    lambda x: (
                        self.son_firmalar.at[x, "ad"] if x != -1 else "Yerleşemedi"
                    )
                )
                ...
                messagebox.showinfo(
                    "Başarılı", f"Rapor başarıyla kaydedildi:\n{dosya_yolu}"
                )
    ...
    def _style_ayarla(self):
        ...
        style.configure(
            "TNotebook.Tab", font=("Segoe UI", 10, "bold"), padding=[35, 10], width=35
        )
    ...
    def _arayuz_kur(self):
        ...
        tk.Label(
            header,
            text="İMEP STAJYER YERLEŞTİRME SİMÜLASYONU",
            fg="white",
            bg="#003366",
        ...
        for lbl, dlt in ayarlar:
            tk.Label(sidebar, text=lbl, bg="white", font=("Segoe UI", 10, "bold")).pack(
                anchor="w", pady=(2, 0)
            )
        ...
        self.combo = ttk.Combobox(
            sidebar,
            values=["Greedy", "Heuristik", "İkisini Karşılaştır"],
            state="readonly",
            style="My.TCombobox",
            font=("Segoe UI", 11),
        )
        ...
        self.btn_csv = tk.Button(
            sidebar,
            text="📁 Firma Listesi (.csv)",
            command=self.csv_sec,
            bg="#e2e8f0",
            relief="flat",
        )
        ...
        self.btn_json = tk.Button(
            sidebar,
            text="📁 Öğrenci Listesi (.json)",
            command=self.json_sec,
            bg="#e2e8f0",
            relief="flat",
        )
        ...
        self.btn_export = tk.Button(
            ...
            fg="white",
            font=("Segoe UI", 11, "bold"),
            command=self.raporu_disa_aktar,
            relief="flat",
            pady=10,
        )
        ...
        self.txt_log.tag_config(
            "tur", foreground="#003366", font=("Consolas", 11, "bold")
        )
        self.txt_log.tag_config(
            "kabul", foreground="#2e7d32", font=("Consolas", 10, "bold")
        )
        self.txt_log.tag_config("red", foreground="#d32f2f")
        self.txt_log.tag_config(
            "info", foreground="#e67e22", font=("Consolas", 10, "bold")
        )

        self._karsilastirma_ekrani_hazirla()
        # Firma tablosu sütun isimlerini belirledik
        self.tree_firma = self._tree_kur(
            self.tab_firma, ("AD", "GNO ŞARTI", "KAPASİTE", "KALAN", "YERLEŞENLER")
        )
        self.tree_res = self._tree_kur(
            self.tab_res, ("ÖĞRENCİ", "GNO", "TUR", "FİRMA", "PUAN")
        )
    ...
    def _tree_kur(self, parent, cols):
        ...
        tk.Label(
            search_frame, text="🔍 Filtrele:", bg="white", font=("Segoe UI", 9, "bold")
        ).pack(side="left")
        ...
        search_var.trace(
            "w", lambda *args: self._tablo_filtrele(tree, search_var.get())
        )
    ...
    def _karsilastirma_ekrani_hazirla(self):
        self.frame_g = tk.LabelFrame(
            self.tab_compare,
            text=" GREEDY SONUÇLARI ",
        ...
        self.frame_h = tk.LabelFrame(
            self.tab_compare,
            text=" HEURİSTİK SONUÇLARI ",
            font=("Segoe UI", 12, "bold"),
            bg="white",
            padx=25,
            pady=25,
    ...
    def csv_sec(self):
        ...
        if yol:
            self.csv_path = yol
            self.btn_csv.config(text="✅ Yüklendi", fg="green")

    def json_sec(self):
        yol = filedialog.askopenfilename(filetypes=[("JSON files", "*.json")])
        if yol:
            self.json_path = yol
            self.btn_json.config(text="✅ Yüklendi", fg="green")
    ...
    def _tablo_filtrele(self, tree, query):
        ...
        source_data = (
            self.all_firma_data if tree == self.tree_firma else self.all_ogrenci_data
        )
        ...
        try:
            ...
            if not (30 <= fs <= 50):
                messagebox.showwarning(
                    "Girdi Hatası", "Firma sayısı 30 ile 50 arasında olmalıdır!"
                )
                return

            if not (100 <= os_val <= 150):
                messagebox.showwarning(
                    "Girdi Hatası", "Öğrenci sayısı 100 ile 150 arasında olmalıdır!"
                )
                return

            if not self.csv_path or not self.json_path:
                messagebox.showwarning("Uyarı", "Lütfen dosyaları seçin!")
                return
            ...
            if secim in ["Heuristik", "İkisini Karşılaştır"]:
                ...
                r2 = simulasyon_motoru(
                    "heuristik", o2, f2, ro, log_yaz, swap_log=self.swap_gui_yaz
                )

                self._update_comp("heuristik", r2)
                final_ogr, final_fir = o2, f2

            # SONUÇLARI TABLOLARA DOLDURMA
            for f_idx, f in final_fir.iterrows():
                ...
                self.tree_firma.insert("", "end", values=satir)

            for _, o in final_ogr.iterrows():
                f_ad = (
                    final_fir.at[o["atanan_firma"], "ad"]
                    if o["atanan_firma"] != -1
                    else "Açıkta"
                )
                satir = (
                ...
                self.tree_res.insert("", "end", values=satir)
